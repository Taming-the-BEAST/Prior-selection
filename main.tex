...
Since we will be using the birth-death skyline model \citep{Stadler2013}, we need to make sure it is available in BEAUti. It is not one of the default models but rather an add-on (also called a plug-in or package). You only need to install a BEAST 2 package once. Thus, if you close BEAUti, you do not have to load \mi{BDSKY} the next time you open the program. However, it is worth checking the package manager for updates to plug-ins, particularly if you update your version of BEAST 2.

\mi{BDSKY} model was used in the previous tutorial, so you should already have it up and running in BEAUti. Also, we do not expect that there were any updates to the version you loaded just few hours ago, so you can skip the following few lines and go directly to section \ref{Importing alignment}. However, in case you have not managed to get it running yet, follow these instructions to install the \mi{BDSKY} add-on. 

\begin{framed}
Open the \mi{BEAST 2 Package Manager} by navigating to \mi{File\textrightarrow Manage Packages}. [Figure \ref{packageManage1}]
\end{framed}

\begin{figure}[h!]
\centering
\fbox{\includegraphics[width=5in]{figures/beast2_package_manager_opening.png}}
\caption{\small Finding the BEAST2 Package Manager.}
\label{packageManage1}
\end{figure}

\begin{framed}
Install the \mi{BDSKY} package by selecting it and clicking the \mi{Install/Upgrade} button. [Figure \ref{packageManage2}]
\end{framed}

\begin{figure}[h!]
\centering
\fbox{\includegraphics[width=5in]{figures/beast2_package_manager.png}}
\caption{\small The BEAST2 Package Manager.}
\label{packageManage2}
\end{figure}

After the installation of an add-on, the program is on your computer, but BEAUti is unable to load the template files for the newly installed model unless it is restarted. So, let's restart BEAUti to make sure we have \mi{BDSKY} model at hand.

\begin{framed}
Close the \mi{BEAST 2 Package Manager} and {\textcolor{red}{\underline{\bf restart BEAUti}}} to fully load the \mi{BDSKY} package.
\end{framed}
...
\begin{framed}
In the \mi{Partitions} panel, import the nexus file with the alignment by navigating to \mi{File\textrightarrow Import Alignment} in the menu [Figure \ref{importAlignment}] and then finding the file called \newline \texttt{InfluenzaAH3N2\_HAgene\_2009\_California\_heterochronous.nexus}  file on your computer.
\end{framed}

\begin{figure}[h!]
\centering
\fbox{\includegraphics[width=5in]{figures/beast2_import_alignment.png}}
\caption{\small Importing alignment into BEAUti.}
\label{importAlignment}
\end{figure}

You can view the alignment by double-clicking on the name of the alignment in BEAUti. Since we only have one partition there is nothing more we can do in the \mi{Partitions} panel and proceed to specifying the tip dates. 
...
\begin{framed}
In the \mi{Tip Dates} panel, click the \mi{Use tip dates} option. 
\end{framed}

We want all the tree information to be specified for units of time in ``years'', thus we leave the \mi{Dates specified as} option set to default ``year''. Also we want the time to flow forward in time in the tree, therefore, we keep to default option of tip dates being specified as ``Since some time in the past'' [Figure \ref{timeUnitsAndFlow}].

\begin{figure}[h!]
\centering
\fbox{\includegraphics[width=5in]{figures/beast2_time_specification.png}}
\caption{\small Specifying time units and direction of time flow.}
\label{timeUnitsAndFlow}
\end{figure}

You could specify the tip dates by hand, by clicking for each row (i.e. for each sequence) into the \mi{Date} column and typing the date information in for each sequence in turn. However, this is a laborious and error-prone procedure and can take a long time to finish. Fortunately, we can use BEAUti, to read off the dates from the sequence names for us. Each sequence is named such that the expression after the last underscore character (``\_'') contains the sampling date information. BEAUti can search for this expression to extract the sequence date.

\begin{framed}
Press the \mi{Guess} button. A window will appear where you can specify how BEAUti can find the date of sampling of each sequence. [Figure \ref{guessDates}]

Select \mi{use everything} and specify \mi{after last} \_.
\end{framed}

\begin{figure}[h!]
\centering
\fbox{\includegraphics[width=5in]{figures/beast2_guess_dates.png}}
\caption{\small Specifying tip dates.}
\label{guessDates}
\end{figure}

You should now see that the tip ages have been filled in for all of the taxa and that the \mi{Date} columns shows a number in form 2009.xyz and the \mi{Height} column shows the number in form 0.abc (the height of the tip from present time, where present is 0.0).
...
\begin{framed}
Navigate to the \mi{Site Model} panel, where we can choose the model of nucleotide evolution that we want to assume to underly our dataset. 
\end{framed}
...
\begin{framed}
Change the \mi{Gamma Category Count} to 4, tick the estimate box next to the \mi{Shape} parameter of the Gamma distribution and set the \mi{Subst Model} to \mi{HKY}. [Figure \ref{substitutionModel}] 
\end{framed}

\begin{figure}[h!]
\centering
\fbox{\includegraphics[width=5in]{figures/beast2_substitution_model.png}}
...
\begin{framed}
Navigate to the \mi{Clock Model} panel.
\end{framed}

Four different clock models are available in BEAST 2, allowing us to specify lineage-specific substitution rate variation. The default model in BEAUti is the \mi{Strict Clock} with a fixed substitution rate equal to 1. The other three models relax the assumption of a constant substitution rate. The \mi{Relaxed Clock Log Normal} allows for the substitution rates associated with each branch to be independently drawn from a single, discretized log normal distribution (\cite{drummond06}). Under the \mi{Relaxed Clock Exponential} model, the rates associated with each branch are drawn from an exponential distribution (\cite{drummond06}). Both of these models are uncorrelated relaxed clock models. The log normal distribution has the advantage that one can estimate its variance, which reflects the extent to which the molecular clock needs to be relaxed. In both models, BEAUti sets by default the \mi{Number Of Discrete Rates} to -1. This means that the number of bins that the distribution is divided into is equal to the number of branches. The last available model is the \mi{Random Local Clock} which averages over all possible local clock models (\cite{drummond10}). 

\begin{framed}
{\bf QUESTION: Which clock model may be the most appropriate for our dataset and why?} 
\end{framed}

Since we are observing the sequence data from a single epidemic of H3N2 virus in humans in a single location (south-west of USA), we do not have a reason to assume different substitution rates for different lineages. Thus, the most straightforward option is to choose the default \mi{Strict Clock} model [Figure \ref{clockModel}]. Note however, that a rigorous model comparison would be the best way to proceed with the choice of the clock model.

\begin{figure}[h!]
\centering
\fbox{\includegraphics[width=5in]{figures/beast2_clock_model.png}}
...
\begin{framed}
Navigate to the \mi{Priors} panel.
\end{framed}
...
\begin{framed}
For the \mi{Tree} model, select the option \mi{Birth Death Skyline Serial}. 

Then, click in the arrow to the left from \mi{R0} to open all the options for R0 settings [Figure \ref{treePrior}]. Leave all the settings to the default, since it specifies not too strong prior on R0 around 1. This is exactly what we want. 

Then, click on the window where it says \mi{initial = [2.0] [0.0, Infinity]}. A pop-up window will show up [Figure \ref{R0Prior}].

In the pop-up window change the \mi{Upper}, the upper limit of the prior distribution, from Infinity to 10 and the \mi{Dimension} of the R0 from 10 to 5 and click \mi{OK}.
\end{framed}

\begin{figure}[h!]
\centering
\fbox{\includegraphics[width=7in]{figures/beast2_prior_R0.png}}
...
\begin{figure}[h!]
\centering
\fbox{\includegraphics[width=4in]{figures/beast2_prior_R0dimension.png}}
\caption{\small Specifying the R0 prior.}
\label{R0Prior}
\end{figure}

Notice that the pop-up window allows one to specify not only the \mi{Dimension} but also the \mi{Minordimension}. If the parameter is specified as a vector of $n$ entries, we only use the \mi{Dimension} with input $n$. If the parameter is specified as an n x m matrix, we then use the \mi{Minordimension} to specify the number of columns (m) the parameter is split into. In the birth-death skyline model, we use the parameter vector only, and thus the \mi{Minordimension} always stays specified as 1.

After we have specified the prior for R0, the next prior that needs our attention is the \mi{becomeUninfectiousRate}. This specifies, how quickly does a person infected with influenza recover. From our personal experience, we would say, it takes around one week to 10 days from the infection of the person to the recovery. This translates to the become uninfectious rate of $365/10=36.5$ to $365/7\approx 52.14$ per year. Let us set the prior for become uninfectious rate accordingly.

\begin{framed}
 Click on the arrow next to the \mi{becomeUninfectiousRate} and change the value for the \mi{M} (mean) of the default log normal distribution to 52 and tick the box \mi{Mean In Real Space} to specify the mean of the distribution in real space instead of a log space [Figure \ref{becomeUninfectiousPrior}].
\end{framed}

\begin{figure}[h!]
\centering
\fbox{\includegraphics[width=7in]{figures/beast2_prior_becomeUninfectious.png}}
...
\begin{framed}
Click on the arrow next to the \mi{clockRate} and change the value for the \mi{M} (mean) of the default log normal distribution to 0.001 and tick the box \mi{Mean In Real Space} [Figure \ref{clockRatePrior}].
\end{framed}

\begin{figure}[h!]
\centering
\fbox{\includegraphics[width=7in]{figures/beast2_prior_clockRate.png}}
...
\begin{figure}[h!]
\centering
\fbox{\includegraphics[width=7in]{figures/beast2_prior_gammaShape.png}}
...
\begin{figure}[h!]
\centering
\fbox{\includegraphics[width=7in]{figures/beast2_prior_kappa.png}}
...
\begin{framed}
Click on the arrow next to the \mi{origin} and change the prior distribution from \mi{Uniform} to \mi{Gamma} with Alpha parameter set to 0.5 and Beta parameter set to 2.0 [Figure \ref{originPrior}].
%
%Make sure that the \mi{Lower} is set to 0.
\end{framed}

\begin{figure}[h!]
\centering
\fbox{\includegraphics[width=7in]{figures/beast2_prior_origin.png}}
...
%\newpage
%Furthermore, the origin value that we expect is positive and most likely not larger than 1. We reflect this in the \mi{origin} specification as follows:
%
%\begin{framed}
%Click on the window where it says \mi{initial = [100.0] [0.0, Infinity]}. A pop-up window will show up [Figure \ref{originPriorPop}].
%
%In the pop-up window make sure that the \mi{Lower}, the lower limit of the prior distribution is set to 0 (do not allow for negative values) and set the \mi{Value} of the origin from 100.0 to 1.0 and click \mi{OK}.
%\end{framed}
%
%\begin{figure}[h!]
%\centering
%\fbox{\includegraphics[width=4in]{figures/beast2_prior_originpop.png}}
...
\begin{framed}
Click on the arrow next to the \mi{samplingProportion} and change the distribution from \mi{Beta} to \mi{Log Normal}. 

Next, change the value for the \mi{M} (mean) to 0.001 and tick the box \mi{Mean In Real Space} [Figure \ref{samplingProportionPrior}].

Also, make sure that the \mi{Lower} is set to 0.0 and the \mi{Upper} is set to 1.0.
\end{framed}

\begin{figure}[h!]
\centering
\fbox{\includegraphics[width=7in]{figures/beast2_prior_samplingProportion.png}}
...
\begin{framed}
Change the \mi{Chain Length} from 10'000'000 to 5'000'000.

Click on the arrow next to the \mi{tree log} and set the \mi{Log Every} to 100'000 [Figure \ref{mcmc}].
\end{framed}

%mcmc chain
\begin{figure}[h!]
\centering
\fbox{\includegraphics[width=5in]{figures/beast2_mcmc.png}}
...
\begin{framed}
Save the XML file as \mi{Heterochronous.xml}
\end{framed}
...
Hit \mi{Run} to start the analysis.
\end{framed}
...
\begin{framed}
Load the file into \mi{Tracer} to check mixing and the parameter estimates. 
\end{framed}
...
Follow the same procedure as for the heterochronous sampling starting at section \ref{Importing alignment}. Now, however, use the alignment file called \texttt{InfluenzaAH3N2\_HAgene\_2009\_California\_homochronous.nexus} and use the \mi{Birth Death Skyline Contemporary} model.
...
\begin{framed}
In \mi{Tracer} click on the \mi{Joint Marginal} panel, select the \mi{TreeHeight} and the \mi{clockRate} simultaneously, and uncheck the \mi{Sample only} box below the graphics [Figure \ref{clockRatetreeHeightCorrelation}].
\end{framed}

\begin{figure}[h!]
\centering
\fbox{\includegraphics[width=6in]{figures/tracer_homochronous_treeHeightclockRatecorrelation.png}}
...
We will use the results from the heterochronous data, to find out what a good estimate for the tree height of these homochronous samples is. For this aim, we first create an MCC (maximum clade credibility) tree in the \mi{TreeAnnotator} and then check with \mi{FigTree} what the estimate of the tMRCA (time to the most recent common ancestor) of the samples from April 28, 2009 is.
...
\begin{framed}
Open the \mi{TreeAnnotator} and set \mi{Burnin percentage} to 10, \mi{Posterior probability limit} to 0.5. Leave the other options unchanged.

Set the \mi{Input Tree File} to \texttt{InfluenzaAH3N2\_HAgene\_2009\_California\_heterochronous.trees} and the \mi{Output File} to \texttt{InfluenzaAH3N2\_HAgene\_2009\_California\_heterochronous.tree}. [Figure \ref{treeAnnotator}]
\end{framed}

\begin{figure}[h!]
\centering
\fbox{\includegraphics[width=3.5in]{figures/treeAnnotator.png}}
\caption{\small Creating the MCC tree.}
\label{treeAnnotator}
\end{figure}

\newpage
How can we find out what the tMRCA of our homochronous data may be? The best may be to have a look at the estimates of the heterochronous data in the \mi{FigTree}.

\begin{framed}
Now open the \mi{FigTree} and load \texttt{InfluenzaAH3N2\_HAgene\_2009\_California\_heterochronous.tree}.

In the upper right corner, next to the magnifier glass sign, type \mi{2009/04/28} to highlight all the sequences from April 28, 2009. [Figure \ref{tMRCAmedian}]
\end{framed}

\begin{figure}[h!]
\centering
\fbox{\includegraphics[width=5in]{figures/FigTree_tMRCA_median.png}}
\caption{\small Displaying median estimates of the node height in the MCC tree.}
\label{tMRCAmedian}
\end{figure}

\begin{framed}
Tick the \mi{Node Labels} in the left menu, and click the arrow next to it to open the full options. Change the \mi{Display} from \mi{age} to \mi{height\_median} [Figure \ref{tMRCAmedian}] and then to \mi{height\_95\%\_HPD} [Figure \ref{tMRCA95HPD}].
\end{framed}

\begin{figure}[h!]
\centering
\fbox{\includegraphics[width=5in]{figures/FigTree_tMRCA_HPD.png}}
...
Create a new taxon set for root node by clicking the \fbox{+} at the bottom of the parameter list in the \mi{Priors} window. This will reveal the \mi{Taxon set  editor}.

Change the \mi{Taxon set} label to \mi{allseq}.

Select the sequences belonging to this clade, i.e. all the tips, and move them from the left column to the right column using the \fbox{> >} button and click \mi{OK}. [Figure \ref{tMRCAPrior}] 
\end{framed}

\begin{figure}[h!]
\centering
\fbox{\includegraphics[width=6in]{figures/beast2_homochronous_tMRCA.png}}
...
\begin{framed}
Back in the \mi{Priors} window, check the box labeled \mi{monophyletic} for the \mi{allseq.prior}.

Click on the arrow next to the \mi{allseq.prior}. Change the prior distribution on the time of the MRCA of selected sequences from \mi{[none]} to \mi{Laplace Distribution} and set the \mi{Mu} to 2008.7745 and the \mi{Scale} to 0.01 [Figure \ref{tMRCAPrior2}]. 

You can check that these settings correspond to the height of tMRCA from the MCC tree by setting \mi{Mu} to 0.5488 and observing the distribution to the right. When you are done, do not forget to set \mi{Mu} back to 2008.7745.
\end{framed}

\begin{figure}[h!]
\centering
\fbox{\includegraphics[width=7in]{figures/beast2_homochronous_tMRCA_prior.png}}
\caption{\small Specifying the root height prior.}
\label{tMRCAPrior2}
\end{figure}
...
\begin{framed}
In the \mi{MCMC} window, click on the arrow next to the \mi{tracelog} and change the \mi{File Name} to \texttt{InfluenzaAH3N2\_HAgene\_2009\_California\_homochronous\_tMRCA.log}.

Then, click on the arrow next to the \mi{treelog} and add \texttt{\_tMRCA} between \texttt{\$(tree)} and \texttt{.trees} in the \mi{File Name} field. [Figure \ref{tMRCAPrior3}]
\end{framed}

\begin{figure}[h!]
\centering
\fbox{\includegraphics[width=5in]{figures/beast2_homochronous_tMRCA_files.png}}
